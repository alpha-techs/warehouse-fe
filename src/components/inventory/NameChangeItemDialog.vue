...
import type { NameChangeItem, InventoryItem } from 'src/api/Api'
...
const emit = defineEmits<{
    ...
    submit: [value: NameChangeItem]
}>();

const {
    itemModel: model,
    inventoryItemModel,
    formModel: nameChangeModel,
...
const showSubDialog = async () => {
    ...
    await subDialog.value?.reload({
        warehouse: nameChangeModel.value.warehouse,
    });
}
...
const onSubmit = (data: InventoryItem) => {
    ...
    model.value.customer = data.customer
...
const onDialogClose = () => {
    ...
    useNameChangeStore().resetInventoryItemModel()
}
</script>
...
<template>
    <div class="q-pa-md q-gutter-sm">
        <q-dialog :model-value="show"
            @update:model-value="(val) => { $emit('update:show', val); if (!val) onDialogClose(); }" persistent>
            <q-card class="card">
                <q-card-section class="bg-primary">
                    <div class="text-h6 text-white">名義変更商品</div>
                </q-card-section>
                <q-card-section>
                    <q-list class="row">
                        <q-item class="col-12">
                            <q-item-section>
                                <q-input :model-value="model.product?.name" label="商品" readonly>
                                    <template v-slot:after>
                                        <q-btn color="primary" round dense flat icon="sym_r_ink_selection"
                                            @click="showSubDialog" />
                                    </template>
                                </q-input>
                            </q-item-section>
                        ...
                        <q-item class="col-12">
                            <q-item-section>
                                <q-input :model-value="model.note"
                                    @update:model-value="model.note = ($event as string || undefined)" label="備考"
                                    autogrow />
                            </q-item-section>
                        </q-item>
                    </q-list>
