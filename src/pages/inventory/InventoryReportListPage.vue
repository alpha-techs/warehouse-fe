...
const generateReport = async () => {
    if (!selectedWarehouse.value || !selectedCustomer.value) {
        await toastFormError('報告を生成するには倉庫と客户を選択してください')
        return
    }
                    ...
                    <q-card-section>
                        ...
                        <div class="row q-col-gutter-md">
                            <div class="col-md-3 col-sm-6 col-xs-12">
                                <q-select v-model="selectedWarehouse" label="倉庫" dense :options="warehouseOptions"
                                    option-label="name" option-value="id" @filter="onFilterWarehouse" clearable
                                    use-input input-style="width: 0px">
                                </q-select>
                            </div>
                            <div class="col-md-3 col-sm-6 col-xs-12">
                                <q-select v-model="selectedCustomer" label="客户" dense :options="customerOptions"
                                    option-label="name" option-value="id" @filter="onFilterCustomer" clearable use-input
                                    input-style="width: 0px">
                                </q-select>
                            </div>
                            <div class="col-md-2 col-sm-6 col-xs-12">
                                <q-select v-model="selectedFormat" label="フォーマット" dense :options="[
                                    { label: 'PDF', value: 'pdf' },
                            ...
                            <div class="col-md-2 col-sm-6 col-xs-12 flex items-center">
                                <q-btn label="報告書生成" color="primary" icon="sym_r_description" @click="generateReport"
                                    :loading="generateLoading" :disable="!selectedWarehouse || !selectedCustomer" />
                            </div>
                        </div>
                    </q-card-section>
                </q-card>
                ...
                <q-card bordered>
                    <q-card-section class="q-pa-none">
                        <q-table flat :columns="columns" :rows="rows" row-key="id" hide-pagination
                            v-model:pagination="pagination" @request="onRequest" ref="tableRef" :loading="loading">
                            <template #top>
                                <div class="row" style="width: 100%">
                                    <div class="text-h6 col-12">在庫報告書一覧</div>
                                    <q-select class="q-px-sm" :model-value="warehouseOptions.find(
                                    ...
                                    <q-select class="q-px-sm" :model-value="customerOptions.find(
                                        (c) => c.id === searchParams.customerId,
                                    )
                                        " @update:model-value="onChangeCustomer" label="客户" dense :options="customerOptions"
                                        option-label="name" option-value="id" @filter="onFilterCustomer" clearable
                                        use-input input-style="width: 0px">
                                    </q-select>
                                    <q-select class="q-px-sm" v-model="searchParams.status" label="ステータス" dense
                                        :options="[
                                    ...
                                    <q-input class="q-px-sm" v-model="searchParams.startDate" label="開始日" dense
                                        type="date" @keyup.enter="search" style="width: 140px">
                                    </q-input>
                                    <q-input class="q-px-sm" v-model="searchParams.endDate" label="終了日" dense
                                        type="date" @keyup.enter="search" style="width: 140px">
                                    </q-input>
                                    <q-space />
                                    <div style="display: flex; align-items: center">
                                        <q-btn size="sm" label="検索" color="primary" icon="sym_r_search"
                                            @click="search" />
                                    </div>
                                </div>
                            </template>
                            <template #[`body-cell-warehouse`]="{ row }: { row: InventoryReport }">
                                <q-td>
                            ...
                            <template #[`body-cell-customer`]="{ row }: { row: InventoryReport }">
                                <q-td>
                                    <a :href="router.resolve({
                            ...
                            <template #[`body-cell-status`]="{ row }: { row: InventoryReport }">
                                <q-td>
                                    <q-badge :color="getStatusColor(row.status || '')"
                                        :label="getStatusLabel(row.status || '')" />
                                </q-td>
                            </template>
                            <template #[`body-cell-format`]="{ row }: { row: InventoryReport }">
                                <q-td>
                                    <q-chip :label="row.format?.toUpperCase()" dense />
                                </q-td>
                            </template>
                            <template #[`body-cell-createdAt`]="{ row }: { row: InventoryReport }">
                                <q-td>
                                    {{
                                        row.createdAt
                            ...
                            <template #[`body-cell-actions`]="{ row }: { row: InventoryReport }">
                                <q-td class="text-right">
                                    <q-btn v-if="row.status === 'completed'" size="sm" flat dense icon="sym_r_download"
                                        @click="downloadReport(row)" />
                                    <q-btn v-else-if="row.status === 'failed'" size="sm" flat dense icon="sym_r_error"
                                        :disable="true" />
                                    <q-btn v-else size="sm" flat dense icon="sym_r_hourglass_empty" :disable="true" />
                                </q-td>
                            </template>
                        </q-table>
                        <q-separator />
