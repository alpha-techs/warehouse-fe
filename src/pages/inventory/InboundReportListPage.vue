...
const {
    inboundReportList: rows,
    inboundReportListPagination,
...
const onRequest = async ({
    ...
    try {
        ...
        await useInboundStore().getInboundReportList(query)
        pagination.value = {
            page: inboundReportListPagination.value.page,
            rowsPerPage: inboundReportListPagination.value.itemsPerPage,
            rowsNumber: inboundReportListPagination.value.totalItems,
            sortBy: undefined,
            descending: false,
        }
    } catch (error) {
...
// Download report
const downloadReport = async (report: InboundReport) => {
    ...
    try {
        ...
        const downloadUrl = useInboundStore().getInboundReportDownloadUrl(
            report.id,
        )
                        ...
                        <q-table flat :columns="columns" :rows="rows" row-key="id" hide-pagination
                            ...
                            <template #top>
                                <div class="row" style="width: 100%">
                                    <div class="text-h6 col-12">入庫報告書一覧</div>
                                    ...
                                    <q-select class="q-px-sm" :model-value="customerOptions.find(
                                        ...
                                        " @update:model-value="onChangeCustomer" label="客户" dense
                            ...
                            <template #[`body-cell-warehouse`]="{ row }: { row: InboundReport }">
                                <q-td>
                            ...
                            <template #[`body-cell-customer`]="{ row }: { row: InboundReport }">
                                <q-td>
                                    <a :href="router.resolve({
                            ...
                            <template #[`body-cell-status`]="{ row }: { row: InboundReport }">
                                <q-td>
                                    <q-badge :color="getStatusColor(row.status || '')"
                                        :label="getStatusLabel(row.status || '')" />
                                </q-td>
                            </template>
                            <template #[`body-cell-format`]="{ row }: { row: InboundReport }">
                                <q-td>
                                    <q-chip :label="row.format?.toUpperCase()" dense />
                                </q-td>
                            </template>
                            <template #[`body-cell-createdAt`]="{ row }: { row: InboundReport }">
                                <q-td>
                                    {{
                                        row.createdAt
                            ...
                            <template #[`body-cell-actions`]="{ row }: { row: InboundReport }">
